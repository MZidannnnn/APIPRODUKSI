                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control" name="tipe_penentuan_waktu" id="tipe_penentuan_waktu">
                                <option value="tanggal" {{ old('tipe_penentuan_waktu', $konfig->tipe_penentuan_waktu ?? 'estimasi') == 'tanggal' ? 'selected' : '' }}>Menggunakan Tanggal</option>
                                <option value="estimasi" {{ old('tipe_penentuan_waktu', $konfig->tipe_penentuan_waktu ?? 'estimasi') == 'estimasi' ? 'selected' : '' }}>Menggunakan Estimasi Pengerjaan</option>
                            </select>
                        </div>
                        
                        <div id="form-tipe-tanggal" class="pl-4 {{ old('tipe_penentuan_waktu', $konfig->tipe_penentuan_waktu ?? 'estimasi') == 'tanggal' ? '' : 'd-none' }}">
                            <div class="form-group mb-2">
                                <label>Batas Minimal Waktu Pemesanan (Hari) :</label>
                                <input type="number" name="batas_hari_zona_merah" class="form-control" placeholder="Contoh: 2 (Untuk H-2)" value="{{ old('batas_hari_zona_merah', $konfig->batas_hari_zona_merah ?? '') }}">
                                <small class="text-muted">Jumlah hari dari sekarang sebagai batas sebelum klien bisa memesan pada kalender.</small>
                            </div>
                        </div>

                        <div id="form-tipe-estimasi" class="pl-4 {{ old('tipe_penentuan_waktu', $konfig->tipe_penentuan_waktu ?? 'estimasi') == 'estimasi' ? '' : 'd-none' }}">
                            <div class="form-group mb-2">
                                <label>Estimasi Pengerjaan :</label>
                                <input type="text" name="estimasi_pengerjaan" class="form-control" placeholder="Contoh: 1-7 hari" value="{{ old('estimasi_pengerjaan', $konfig->estimasi_pengerjaan ?? '') }}">
                            </div>
                        </div>
                    </div>
                </div>
