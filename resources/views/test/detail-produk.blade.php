                        ...
                        <div class="swiper-wrapper">
                            @foreach ($itemProduksi->fotoProduk as $foto)
                                <div class="swiper-slide">
                                    <img src="{{ asset($foto->nama_foto) }}" alt="Foto {{ $loop->iteration }}" loading="lazy">
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="swiper-wrapper">
                            @foreach ($itemProduksi->fotoProduk as $foto)
                                <button class="swiper-slide thumb" type="button">
                                    <img src="{{ asset($foto->nama_foto) }}" alt="Thumb {{ $loop->iteration }}" loading="lazy">
                                </button>
                            @endforeach
                        </div>
                    </div>
            ...
            @if ($detailDefault)
                <div class="item-detail">
                    ...
                    <div>
                        ...
                        <span class="value price">Rp{{ number_format($detailDefault->harga_dasar, 0, ',', '.') }}</span>
                    </div>
                </div>
            @endif

            @auth
                <a class="btn btn-outline chat-cta" href="{{ route('chat.show', $percakapan->id_percakapan) }}">Chat Admin</a>
            @endauth
        </section>
                    ...
                    <select name="id_detail_produk" required>
                        @foreach ($itemProduksi->detailProduk as $detail)
                            <option value="{{ $detail->id_detail_produk }}">
                                {{ $detail->ukuran ?? 'Standar' }} - Rp{{ number_format($detail->harga_dasar, 0, ',', '.') }}
                            </option>
                        @endforeach
                    </select>
                </div>
